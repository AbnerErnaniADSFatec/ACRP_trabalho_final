...
def collection_get_data(stac, datacube, data_dir):
    ...
    for item in item_search.items():
        ...
        if (
            ...
            or collection == "landsat-2"
            or collection == "S2-16D-2"
        ):
    ...
    for item in item_search.items():
        ...
        for band in bands:
            ...
            if not any(
                tile_dict["tile"] == tile
                for tile_dict in geom_map
            ):
                geom_map.append(
                    dict(tile=tile, geometry=item.geometry)
                )

            output_file = os.path.join(
                data_dir + "/" + collection + "/" + tile + "/" + band,
                os.path.basename(item.assets[band].href)
            )
            ...
            if os.path.exists(output_file):
                ...
                download_stream(
                    output_file,
                    response,
                    total_size=item.to_dict()['assets'][band]["bdc:size"]
                )
    ...
    if download:
        ...
        with open(
            os.path.join(
                data_dir + "/" + collection + "/" + file_name
            ),
            'w'
        ) as json_file:

            json.dump(
                dict(collection=collection, geoms=geom_map),
                json_file,
                indent=4
            )
    ...
    print(
        ...
        f"{item_search.matched()} scenes "
        f"to {os.path.join(collection)}"
    )
...
def simple_cube(stac_url, collection, start_date, end_date, tile=None, bbox=None, freq=None, bands=None):
    ...
    if (collection_name == "samet_daily-1" or collection_name == "prec_merge_daily-1"):
        ...
        with rasterio.open(sample_image_path) as src:
            data_proj = src.crs
        if data_proj is None or not data_proj:
            data_proj = pyproj.CRS.from_epsg(4326)
